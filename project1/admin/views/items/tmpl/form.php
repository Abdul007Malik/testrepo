...
<form action="<?php echo JRoute::_('index.php?option=com_helloworld&view=items&layout=form&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="adminForm" class="form-validate"enctype="multipart/form-data">
    ...
    <div class="form-horizontal">
		...
		<fieldset class="adminform">
				...
				<div class="row-fluid">
					<div class="span6">
						<label class="inputbox"><?php echo JText::_('COM_HELLOWORLD_ITEM_FIELD_SHOW_CATEGORY_LABEL') ?></label>
						<Select name="show_category" class="inputbox">

							<?php
								if(isset($this->item->params['show_category']))
								{
									echo "<option value=''". (($this->item->params['show_category']==='')?"selected='selected'>" :">").JText::_("JGLOBAL_USE_GLOBAL")."</option>";
									echo "<option value='1'". (($this->item->params['show_category']==='1')?"selected='selected'>" :">")."JSHOW</option>";
									echo "<option value='0'". (($this->item->params['show_category']==='0')?"selected='selected'>" :">")."JHIDE</option>";

								}else{
									echo "<option value='' selected='selected'>JGLOBAL_USE_GLOBAL</option>";
									echo "<option value='0'>JHIDE</option>
