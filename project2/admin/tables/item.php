...
class HelloWorldTableItem extends JTable
{
	...
	{
		parent::__construct('#__hwitems', 'id', $db);
	}
	...
	{
		$k = $this->_tbl_key;
		return 'com_helloworld.hwitems.'.(int) $this->$k;
	}
	/**
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL)
	{
		...
		if (($this->catid)&& !empty($this->catid))
		{
			...
			$assetParent->loadByName('com_helloworld.hwcategories.' . (int) $this->catid);
		}
		else
		{
	...
	function store($updateNulls = false)
	{
		...
		if($files){
			...
			foreach($files as $i=>$file){
				...
				if($tmpFilePath!=""){
				$newFilePath=JPATH_COMPONENT_SITE."/assets/upload/item/images/".$name;
				$flag=JFile::upload($tmpFilePath, $newFilePath);
				$this->createThumbnail($name, $tw, $tp);
				$this->createThumbnail($name, $twlr, $tplr);
				}
				$images[$i]=$name;
			}
	...
	public function createThumbnail($filename, $iw, $tp) {
		...
		$path_to_image_directory = JPATH_COMPONENT_SITE."/assets/upload/item/images/";
		$path_to_thumbs_directory = JPATH_COMPONENT_SITE."/assets/upload/item/images/".$tp;
	...
	* Method  to delete the Image in the row*/
	...
	public function deleteImg($img){
		  ...
		  $key=array_search($img,$pre);
			if($pre[$key]==$img){
				$image=JPATH_COMPONENT_SITE.DIRECTORY_SEPARATOR."/assets/upload/item/images/";
