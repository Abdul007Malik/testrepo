 ...
 * @since  0.0.1
 */
class HelloWorldModelCategory extends JModelLegacy
{
	/**
	...
	protected function canDelete($record)
	{
		if( !empty( $record->id ) )
		{
			return JFactory::getUser()->authorise( "core.delete", "com_helloworld.category." . $record->id );
		}
	}
	...
	function save($data)
	{
		$table = $this->getTable('Category', 'HelloWorldTable');
	...
	public function validate($data){
		...
		$target_dir = "uploads/";
		$target_file = $target_dir . basename($_FILES["images[0]"]["name"]);
		$uploadOk = 1;
		...
		if(isset($_POST["submit"])) {
			...
			if($check !== false) {
				...
				$this->setError("File is not an image");
				return false;
			}
			 // Check file size
		...
		if(!empty($data['title'])){
            ...
            $test = preg_match($regex,$data['title']);
			if(!$test || $test=='0'){

				$this->setError("Category title must notstart with numerals");
				return false;
			}
		}else{
