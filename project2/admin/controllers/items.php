...
class HelloWorldControllerItems extends JControllerLegacy
{
	...
	public function deleteImg()
	{
		...
		if(!$model->deleteImg($img,$recordId)){
			...
			$this->setRedirect(
			    JRoute::_(
						'index.php?option=' . $this->option . '&view=items&layout=form &id=' . $recordId, false
					)
			);
			$return->result=false;
	...
	{	// Check for request forgeries.
		...
		if(isset($data['show_category'])){
			$data['params']=array('show_category'=>$data['show_category']);
			unset($data['show_category']);
		}
		...
		$validData['detail'] = JRequest::getVar('detail','','post','string',JREQUEST_ALLOWHTML);
		...
		switch ($task)
		{
			...
			case 'save2new':
				...
				$this->setRedirect(
				    JRoute::_(
						'index.php?option=' . $this->option . '&view=items&layout=form', false
					)
				);
				break;
			default:
				...
				$this->releaseEditId($context, $recordId);
				$app->setUserState($context . '.data', null);
				$url = 'index.php?option=' . $this->option . '&view=items';
	...
	*/
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if( !empty( $id ) )
		{
			return JFactory::getUser()->authorise( "core.edit", $this->option . ".item." . $id );
		}
	}
	...
	function cancel()
	{
		...
		$this->setRedirect( 'index.php?option=com_helloworld&view=items', $msg );
